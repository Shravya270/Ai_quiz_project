...
export default function Home() {
  ...
  return (
            ...
            <div className="grid grid-cols-1 gap-6 mt-6">
                {
                    questions[currentQuestion].answers.map(answer=>{
                      const variant = selectedAnswer === answer.id ? (answer.isCorrect?"neoSuccess":"neoDanger"):"neoOutline";
                        return (
                            <Button key={answer.id} variant={variant} size="xl" onClick={()=> handleAnswer(answer)}>{answer.answerText}</Button>
                        )
                    })
                }
            </div>
    ...
    <footer className="footer pb-9 px-6 relative mb-0">
        ...
        <Button variant="neoOutline" size="lg" onClick={handleNext}>{!started ? 'Start': (currentQuestion === questions.length-1)? 'Submit':'Next'}</Button>
    </footer>
    </div>
  )
}
