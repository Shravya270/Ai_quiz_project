...
const neoClasses =
  "w-full rounded-full px-1 py-1 border-2 relative z-10 text-lg font-bold hover:transform hover:translate-y-[-2px] transition-transform duration-200";
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant, size, asChild = false, ...props }, ref) => {
    ...
    const showShadow = variant === "neo" || variant === "neoOutline" ||variant==="neoSuccess"||variant==="neoDanger";
    ...
    return (
      <div className={cn("relative inline-block w-full", { "pb-2": showShadow })}>
        {showShadow && (
          <span
            className={cn(spanVariants({ variant }), borderRound)}
          />
        )}
